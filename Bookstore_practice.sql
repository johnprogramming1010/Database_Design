CREATE DATABASE BOOKstore_practice;
USE BOOKstore_practice;
...
SELECT S.BookID, B.Title, C.state, SUM(S.Quantity) AS TotalQuantity, SUM(S.Quantity * S.UnitPrice) AS Totalsales
FROM Sale AS S
INNER JOIN Customer AS C ON S.CustID = C.ID
INNER JOIN Book AS B ON S.BookID = B.ID
WHERE (C.State = 'CO' OR C.State = 'OK') AND MONTH(S.Date) = 2 AND YEAR(S.Date) = 2020 AND S.BookID IN 
	(SELECT BookID
    FROM BookAuthor
    GROUP BY BookID
    HAVING COUNT(AuthorID) = 1)
GROUP BY S.BookID, B.Title, C.State
